    ...
    public class LinearMissileSkillshot : EvadeSkillshot
    {
        ...
        public Vector3 EndPosition
        {
            get
            {
                ...
                {
                    return Missile.StartPosition.ExtendVector3(Missile.Position, SpellData.Range);
                }
            }
        }
        ...
        public override int GetAvailableTime(AIHeroClient champion = null)
        {
            champion = champion ?? Player.Instance;

            var dist1 =
                Math.Abs((EndPosition.Y - StartPosition.Y)*champion.ServerPosition.X -
                         (EndPosition.X - StartPosition.X)*champion.ServerPosition.Y +
                         EndPosition.X*StartPosition.Y - EndPosition.Y*StartPosition.X)/
                (StartPosition.To2D().Distance(EndPosition.To2D()));

            var actualDist =
                Math.Sqrt(Extensions.Pow(StartPosition.To2D().Distance(champion.ServerPosition.To2D())) -
                          Extensions.Pow(dist1));
